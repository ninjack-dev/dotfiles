...
# The added option is --input-mode. Due to how kittens handle input parsing, instead of using PanelCLIOptions,
# CLIOptions was used instead as it is more generic; PanelCLIOptions pulls from the original panel Python file, when it would
# be better to pass it through ourselves. Perhaps there is a better way to do this, as the Pyright type checking
# freaks out when trying to access its types (likely a cache issue...)

import sys
from typing import Any, Callable, Dict, List, Tuple

from kitty.cli import CLIOptions, parse_args
...
choices=0,1,2
default=0
Which wayland layer focus policy to apply to the layer that this panel will be in.
0 - Focus not allowed
...
def parse_panel_args(args: List[str]) -> Tuple[CLIOptions, List[str]]:
...
def layer_shell_config(opts: CLIOptions) -> LayerShellConfig:
    ...
    return LayerShellConfig(type=ltype, edge=edge, focus_policy=opts.input_mode, size_in_cells=max(1, opts.lines), output_name=opts.output_name or '')
