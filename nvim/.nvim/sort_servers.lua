...
local function sort_servers_table()
  local bufnr = vim.fn.bufnr(lsp_file, true)
  if bufnr == -1 then
    vim.notify("lsp.lua not found in current directory", vim.log.levels.ERROR)
    return
  end
  ...
  for _, v in ipairs(root:field("local_declaration")) do
    if
      vim.treesitter.get_node_text(v:child(1):child(0):field("name")[1], bufnr) == "servers"
      and v:child(1):child(2)
      and v:child(1):child(2):type() == "expression_list"
    then
  ...
  for _, entry in ipairs(entries) do
    ...
    for s in
      (vim.treesitter.get_node_text(entry.entry, bufnr):gsub("^%s*", string.rep(" ", start_column)) .. ","):gmatch(
        "[^\r\n]+"
      )
    do
...
vim.api.nvim_create_autocmd("BufWritePre", {
  pattern = lsp_file, -- relative to CWD or use full path
