...
---@class markdown_codeblock_options
...
local function select_language(language_name_map, language, confirm_substitution)
  ...
  if global_map_result ~= nil and type(global_map_result) == "string" then
    if
      ...
      and vim.fn.confirm(
          string.format("Syntax highlight substitution found: %s --> %s. Accept?", language, global_map_result),
          "&Yes\n&No",
          1
        )
        ~= 1
...
---@param opts? markdown_codeblock_options
function M.markdown_codeblock(opts)
  opts = opts or {}
  if opts.add_filename_comment == nil then
    opts.add_filename_comment = true
  end
  if opts.confirm_language_substitution == nil then
    opts.confirm_language_substitution = false
  end
  if opts.source_register == nil then
    opts.source_register = "0"
  end
  if opts.target_register == nil then
    opts.target_register = "+"
  end
